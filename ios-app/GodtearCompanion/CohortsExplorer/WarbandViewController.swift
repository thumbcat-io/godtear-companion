...
class WarbandViewController: UIViewController, UIScrollViewDelegate {
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        if scrollView === self.scrollView {
            ...
            if y == 0 {
                self.navigationItem.title = cohort.champion.name
            } else if y == scrollView.frame.height {
                self.navigationItem.title = cohort.follower.name
            } else {
...
extension WarbandViewController {
